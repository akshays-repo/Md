...
export class GoogleComponent extends Component {
  ...
  getInfo(param) {
    ...
    if (this.props.apiKey) {
        ...
        .then(dataJson => {
          return dataJson.json().then(data => {
            if (data.status == 'OK' && data.predictions.length > 0) {
              ...
              child.push(
                React.createElement('li', { className: this.state.liStyle }, data.error_message),
              );
            } else {
              child.push(
                React.createElement('li', { className: this.state.liStyle }, 'NO Result Found'),
              );
            }
            let collection = React.createElement(
              'ul',
      ...
      child.push(
        React.createElement('li', { className: this.state.liStyle }, 'No Api Key Provided'),
      );
      let collection = React.createElement('ul', { className: 'style-unordered-list' }, child);
      this.setState({ collection: collection, collectionShow: true });
    }
  }
  ...
  render() {
    return React.createElement(
      'div',
      { className: 'location-box-cover', ref: node => this.setWrapperRef(node) },

      React.createElement('i', {
        className: 'fal fa-map-marker-alt mapicon',
      }),
